...
const ResultsPageV2 = ({ 
  ...
  const [compIncludeSale, setCompIncludeSale] = useState(true);
  const [compDisplayMode, setCompDisplayMode] = useState('monetary');
  const [compProfitView, setCompProfitView] = useState('table');
  const [compCapStructYear, setCompCapStructYear] = useState(0);
...
5. SUMMARY FOR ME
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'compressed': {
        // ════════════════════════════════════════════════════════════════
        // COMPRESSED — One-page investment summary dashboard
        // ════════════════════════════════════════════════════════════════
        const cB = '#e5e7eb', cAC = '#4f46e5', cLB = '#6b7280', cVL = '#111827';
        const cSC = { backgroundColor: '#fff', borderRadius: 16, padding: '24px 28px', marginBottom: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: `1px solid ${cB}` };
        ...
        // ── Data sources ──
        const cExitScenarios = fullCalcs.returns?.exitScenarios || [];
        const cProjections = fullCalcs.projections || [];
        const cDebtTimeline = (fullCalcs.exit?.debtTimeline || []);
        const cEquityTimeline = fullCalcs.exit?.equityExitTimeline || { rows: [] };
        const cEquityRows = cEquityTimeline.rows || [];
        const cSelectedScenario = cExitScenarios.find(s => s.exitYear === selectedHoldPeriod) || cExitScenarios[0] || {};
        const cTotalEquity = fullCalcs.financing?.totalEquityRequired || fullCalcs.total_project_cost || 0;
        const cLoanAmount = fullCalcs.financing?.loanAmount || 0;
        const cIRR = fullCalcs.returns?.leveredIRR || 0;
        const cEquityMultiple = fullCalcs.returns?.equityMultiple || cSelectedScenario.equityMultiple || 0;
        const cCoC = fullCalcs.year1?.cashOnCash || 0;
        const cTotalProfit = cSelectedScenario.totalProfit || 0;
        const cStartingNOI = fullCalcs.year1?.noi || noiT12 || 0;
        const cClosingCosts = fullCalcs.acquisition?.closingCosts || (purchasePrice * 0.02) || 0;
        const cAcquisitionCost = fullCalcs.acquisition?.totalAcquisitionCosts || (purchasePrice + cClosingCosts);

        // ── Purchase price slider ──
        const cMinPrice = Math.round(purchasePrice * 0.4);
        const cMaxPrice = Math.round(purchasePrice * 1.6);
        const cCurrentPrice = purchasePrice;

        // ── Cap rate sensitivity ──
        const cBaseCapRate = capRate > 0 ? capRate : 5.0;
        const cCapRates = [];
        const cBaseIdx = 3;
        for (let i = 0; i < 7; i++) {
          cCapRates.push(Number((cBaseCapRate + (i - cBaseIdx) * 0.25).toFixed(2)));
        }

        // Optimized NOI = stabilized / value-add NOI
        const cOptimizedNOI = fullCalcs.stabilized?.noi || (cStartingNOI * 1.15);

        // ── Yearly data for cash flow chart + profitability table ──
        const cYearlyData = [];
        const cHoldYears = selectedHoldPeriod || 10;
        const cAnnualGrowth = scenarioData.growth?.income_growth || 0.03;
        const cAnnualDebtSvc = annualDebtService || 0;

        for (let yr = 1; yr <= Math.min(cHoldYears, 10); yr++) {
          const proj = cProjections.find(p => p.year === yr);
          const exitSc = cExitScenarios.find(s => s.exitYear === yr);
          const eqRow = cEquityRows[yr - 1];
          const debtRow = cDebtTimeline[yr - 1];

          const yrNOI = proj?.noi || (cStartingNOI * Math.pow(1 + cAnnualGrowth, yr));
          const yrCashFlow = eqRow?.totalDistribution || (yrNOI - cAnnualDebtSvc);
          ...
          // Increase in Value = sale value minus purchase price (only meaningful at exit)
          const increaseInValue = 0;

          // Net worth increase = cash flow + principal paydown + increase in value
          ...
          cYearlyData.push({
            ...
            netWorthIncrease: netWorthIncrease,
            cumulativeCashFlow: 0, // will compute below
          });
        }
        // Cumulative cash flow
        let cRunningCF = 0;
        cYearlyData.forEach(d => { cRunningCF += d.cashFlow; d.cumulativeCashFlow = cRunningCF; });

        // Averages for profitability
        const cAvgCashFlow = cYearlyData.length > 0 ? cYearlyData.reduce((s, d) => s + d.cashFlow, 0) / cYearlyData.length : 0;
        const cAvgPrincipal = cYearlyData.length > 0 ? cYearlyData.reduce((s, d) => s + d.principalPaydown, 0) / cYearlyData.length : 0;
        const cAvgIncValue = 0;
        const cAvgNetWorth = cYearlyData.length > 0 ? cYearlyData.reduce((s, d) => s + d.netWorthIncrease, 0) / cYearlyData.length : 0;

        // Capital structure per year
        const cCapStructData = (() => {
          const yr = compCapStructYear;
          if (yr === 0) {
            return { equity: cTotalEquity, debt: cLoanAmount, ltc: cLoanAmount > 0 ? (cLoanAmount / cAcquisitionCost) * 100 : 0, dscr: dscr };
          }
          const debtRow = cDebtTimeline[yr - 1];
          const bal = debtRow?.endingBalance || cLoanAmount;
          const eqVal = cAcquisitionCost - bal;
          return { equity: eqVal > 0 ? eqVal : cTotalEquity, debt: bal, ltc: cAcquisitionCost > 0 ? (bal / cAcquisitionCost) * 100 : 0, dscr: dscr };
        })();
        const cTotalCapital = cCapStructData.equity + cCapStructData.debt;
        const cEquityPct = cTotalCapital > 0 ? Math.round((cCapStructData.equity / cTotalCapital) * 100) : 0;
        const cDebtPct = cTotalCapital > 0 ? Math.round((cCapStructData.debt / cTotalCapital) * 100) : 0;

        // Total Investment Return
        const cCumCashFlows = cYearlyData.reduce((s, d) => s + d.cashFlow, 0);
        const cNetSalePrice = cProjections.find(p => p.year === selectedHoldPeriod)?.grossSalesPrice || cSelectedScenario.salePrice || 0;
        const cLoanBalAtExit = cProjections.find(p => p.year === selectedHoldPeriod)?.loanBalance || (cDebtTimeline[selectedHoldPeriod - 1]?.endingBalance) || 0;
        const cTotalCashReceived = cCumCashFlows + cNetSalePrice - Math.abs(cLoanBalAtExit);
        const cFinancedByDebt = cLoanAmount;
        const cInitialInvestment = 0;
        const cTotalCashInvested = purchasePrice + cClosingCosts + cInitialInvestment - cFinancedByDebt;
        const cCompTotalProfit = cTotalCashReceived - cTotalCashInvested;

        // ── Profitability: with/without sale, monetary/percentage ──
        const cProfitRows = [
          {
            label: 'Cash Flow',
            avg: cAvgCashFlow,
            values: cYearlyData.map(d => d.cashFlow),
            pctBase: cTotalEquity,
          },
          {
            label: 'Principal Paydown',
            avg: cAvgPrincipal,
            values: cYearlyData.map(d => d.principalPaydown),
            pctBase: cTotalEquity,
          },
          {
            label: 'Increase in Value',
            avg: cAvgIncValue,
            values: cYearlyData.map(d => d.increaseInValue),
            pctBase: cTotalEquity,
          },
          {
            label: 'Increase in Networth',
            avg: cAvgNetWorth,
            values: cYearlyData.map(d => d.netWorthIncrease),
            pctBase: cTotalEquity,
            highlight: true,
          },
        ];
        ...
        const cFormatVal = (v) => {
          if (compDisplayMode === 'monetary') return cFmt(Math.round(v));
          return cPct(cTotalEquity > 0 ? (v / cTotalEquity) * 100 : 0);
        };
        ...
        return (
          <div style={{ padding: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 0, marginBottom: 24 }}>
              <div style={cSC}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                  <div style={{ width: 4, height: 24, backgroundColor: cAC, borderRadius: 2 }} />
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: cVL }}>Overview</h3>
                </div>
                {/* Top metrics row */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 24 }}>
                  {[
                    { label: 'IRR', value: `${(cIRR * 100).toFixed(0)}%`, tip: 'Internal Rate of Return — annualized return accounting for time value of money' },
                    { label: 'Total Potential Profit', value: cFmt(cTotalProfit), tip: 'Cumulative profit including cash flow, principal paydown, and sale proceeds' },
                    { label: 'Cash on Cash Return', value: `${cCoC.toFixed(0)}%`, tip: 'Year 1 cash flow divided by total equity invested' },
                    { label: 'Equity Multiple', value: `${cEquityMultiple.toFixed(2)}x`, tip: 'Total cash returned divided by total equity invested' },
                  ].map((m, i) => (
                    <div key={i}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
                        <span style={{ fontSize: 12, color: cLB, fontWeight: 600 }}>{m.label}</span>
                        <span title={m.tip} style={{ width: 14, height: 14, borderRadius: '50%', border: `1px solid ${cB}`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: cLB, cursor: 'help' }}>?</span>
                      </div>
                      <div style={{ fontSize: 28, fontWeight: 800, color: cVL }}>{m.value}</div>
                    </div>
                  ))}
                </div>

                {/* Purchase Price slider */}
                <div style={{ background: '#f9fafb', borderRadius: 12, padding: '20px 24px', border: `1px solid ${cB}` }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: cVL, textAlign: 'center', marginBottom: 12 }}>Purchase Price</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'center', marginBottom: 12 }}>
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 14, color: cLB, fontWeight: 600 }}>$</span>
                      <input
                        type="text"
                        value={cCurrentPrice.toLocaleString()}
                        onChange={(e) => {
                          const v = parseInt(e.target.value.replace(/[^0-9]/g, '')) || 0;
                          handleFieldChange('pricing_financing.purchase_price', v);
                          handleFieldChange('pricing_financing.price', v);
                        }}
                        style={{ width: 180, padding: '10px 12px 10px 28px', fontSize: 18, fontWeight: 700, border: `1.5px solid ${cAC}`, borderRadius: 10, outline: 'none', textAlign: 'center', color: cVL }}
                      />
                    </div>
                    <button
                      onClick={() => {
                        handleFieldChange('pricing_financing.purchase_price', cCurrentPrice);
                        handleFieldChange('pricing_financing.price', cCurrentPrice);
                      }}
                      style={{ padding: '10px 20px', fontSize: 13, fontWeight: 600, color: cAC, background: 'none', border: 'none', cursor: 'pointer' }}
                    >Save</button>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{ fontSize: 12, color: cLB, fontWeight: 600, minWidth: 80, textAlign: 'right' }}>{cFmt(cMinPrice)}</span>
                    <input
                      type="range"
                      min={cMinPrice}
                      max={cMaxPrice}
                      ...
                      style={{ flex: 1, accentColor: cAC }}
                    />
                    <span style={{ fontSize: 12, color: cLB, fontWeight: 600, minWidth: 80 }}>{cFmt(cMaxPrice)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 16 }}>
                    <button onClick={() => navigate('/loi-generator')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 18px', fontSize: 12, fontWeight: 600, color: cVL, background: '#fff', border: `1px solid ${cB}`, borderRadius: 8, cursor: 'pointer' }}>
                      <span style={{ fontSize: 14 }}>+</span> Create new LOI
                    </button>
                    <button onClick={() => { /* Scroll to exit strategy or open refi modal */ setActiveTab('exit-strategy'); }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 18px', fontSize: 12, fontWeight: 600, color: cVL, background: '#fff', border: `1px solid ${cB}`, borderRadius: 8, cursor: 'pointer' }}>
            ...
            <div style={cSC}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                <div style={{ width: 4, height: 24, backgroundColor: cVL, borderRadius: 2 }} />
                <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: cVL }}>Project Valuation</h3>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr>
                      <th style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 700, color: cLB, fontSize: 12, borderBottom: `2px solid ${cB}` }}>Cap Rate</th>
                      {cCapRates.map((cr, i) => (
                        <th key={i} style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 700, fontSize: 12, borderBottom: `2px solid ${cB}`, backgroundColor: i === cBaseIdx ? `${cAC}10` : 'transparent', color: i === cBaseIdx ? cAC : cLB, borderRadius: i === cBaseIdx ? '8px 8px 0 0' : 0 }}>
                          {cr.toFixed(2)}%
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    <tr>
                      <td style={{ padding: '10px 14px', fontWeight: 600, color: cLB, fontSize: 12, borderBottom: `1px solid ${cB}` }}>Based on Starting NOI</td>
                      {cCapRates.map((cr, i) => {
                        const val = cr > 0 ? cStartingNOI / (cr / 100) : 0;
                        return (
                          <td key={i} style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 600, color: i === cBaseIdx ? cAC : cVL, backgroundColor: i === cBaseIdx ? `${cAC}10` : 'transparent', borderBottom: `1px solid ${cB}`, fontSize: 12 }}>
                            {cFmt(Math.round(val))}
                          </td>
                        );
                      })}
                    ...
                    <tr>
                      <td style={{ padding: '10px 14px', fontWeight: 600, color: cLB, fontSize: 12 }}>Based on Optimized NOI</td>
                      {cCapRates.map((cr, i) => {
                        const val = cr > 0 ? cOptimizedNOI / (cr / 100) : 0;
                        return (
                          <td key={i} style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 700, color: i === cBaseIdx ? cAC : cVL, backgroundColor: i === cBaseIdx ? `${cAC}10` : 'transparent', fontSize: 12 }}>
                            {cFmt(Math.round(val))}
                          </td>
                        );
                      })}
            ...
            <div style={cSC}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 4, height: 24, backgroundColor: cVL, borderRadius: 2 }} />
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: cVL }}>Yearly Cash Flow</h3>
                </div>
                <button onClick={() => setActiveTab('cashflow')} style={{ fontSize: 12, color: cLB, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>View Details &rsaquo;</button>
              </div>
              <div style={{ width: '100%', height: 280 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={cYearlyData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                    <defs>
                      <linearGradient id="cfGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={cAC} stopOpacity={0.15} />
                        <stop offset="95%" stopColor={cAC} stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="label" tick={{ fontSize: 11, fill: cLB }} axisLine={false} tickLine={false} />
                    <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 11, fill: cLB }} axisLine={false} tickLine={false} width={60}
                      domain={['auto', 'auto']} />
                    <Tooltip formatter={(v) => [cFmt(Math.round(v)), 'Cash Flow']} labelStyle={{ fontWeight: 700 }} />
                    <Area type="monotone" dataKey="cashFlow" stroke={cAC} strokeWidth={2.5} fill="url(#cfGrad)" dot={{ r: 3, fill: cAC }} activeDot={{ r: 5 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              {/* Y-axis labels on right */}
              {cYearlyData.length > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
                  <span style={{ fontSize: 11, color: cLB }}>{cFmt(Math.round(Math.min(...cYearlyData.map(d => d.cashFlow))))}</span>
                  <span style={{ fontSize: 11, fontWeight: 700, color: cVL }}>{cFmt(Math.round(Math.max(...cYearlyData.map(d => d.cashFlow))))}</span>
                </div>
              )}
            </div>
            ...
            <div style={cSC}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 4, height: 24, backgroundColor: cVL, borderRadius: 2 }} />
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: cVL }}>Profitability</h3>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  {/* Include Sale toggle */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 12, color: cLB, fontWeight: 600 }}>Include Sale</span>
                    <div onClick={() => setCompIncludeSale(!compIncludeSale)} style={{ width: 40, height: 22, backgroundColor: compIncludeSale ? cAC : '#d1d5db', borderRadius: 11, padding: 2, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: compIncludeSale ? 'flex-end' : 'flex-start', transition: 'background 0.2s' }}>
                      <div style={{ width: 18, height: 18, backgroundColor: '#fff', borderRadius: '50%', boxShadow: '0 1px 3px rgba(0,0,0,0.25)' }} />
                    </div>
                  </div>
                  {/* Monetary / Percentage selector */}
                  <select value={compDisplayMode} onChange={(e) => setCompDisplayMode(e.target.value)} style={{ fontSize: 12, padding: '4px 8px', border: `1px solid ${cB}`, borderRadius: 6, color: cVL, fontWeight: 600, cursor: 'pointer' }}>
                    <option value="monetary">Monetary</option>
                    <option value="percentage">Percentage</option>
                  </select>
                  {/* Table / Chart toggle */}
                  <div style={{ display: 'flex', border: `1px solid ${cB}`, borderRadius: 6, overflow: 'hidden' }}>
                    <button onClick={() => setCompProfitView('table')} style={{ padding: '4px 10px', fontSize: 12, fontWeight: 600, background: compProfitView === 'table' ? cAC : '#fff', color: compProfitView === 'table' ? '#fff' : cLB, border: 'none', cursor: 'pointer' }}>&#9638;</button>
                    <button onClick={() => setCompProfitView('chart')} style={{ padding: '4px 10px', fontSize: 12, fontWeight: 600, background: compProfitView === 'chart' ? cAC : '#fff', color: compProfitView === 'chart' ? '#fff' : cLB, border: 'none', cursor: 'pointer' }}>&#9636;</button>
                  </div>
                </div>
              </div>

              {compProfitView === 'table' ? (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ borderBottom: `2px solid ${cB}` }}>
                        <th style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 700, color: cLB, minWidth: 140 }}></th>
                        <th style={{ padding: '10px 14px', textAlign: 'right', fontWeight: 700, color: cLB }}>Average</th>
                        {cYearlyData.map((d, i) => (
                          <th key={i} style={{ padding: '10px 14px', textAlign: 'right', fontWeight: 700, color: cLB, minWidth: 90 }}>
                            <div>Year {d.year}</div>
                    ...
                    <tbody>
                      {cProfitRows.map((row, ri) => (
                        <tr key={ri} style={{ backgroundColor: row.highlight ? `${cAC}08` : (ri % 2 === 0 ? '#fff' : '#f9fafb'), borderBottom: `1px solid ${cB}` }}>
                          <td style={{ padding: '10px 14px', fontWeight: row.highlight ? 700 : 600, color: row.highlight ? cAC : cVL }}>{row.label}</td>
                          <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: 600, color: cVL }}>{cFormatVal(row.avg)}</td>
                          {row.values.map((v, vi) => (
                            <td key={vi} style={{ padding: '10px 14px', textAlign: 'right', fontWeight: row.highlight ? 700 : 500, color: row.highlight ? cAC : (v > 0 ? cVL : v < 0 ? '#ef4444' : cLB) }}>
                              {cFormatVal(v)}
                            </td>
                          ))}
                        </tr>
                ...
                <div style={{ width: '100%', height: 300 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={cYearlyData} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      <XAxis dataKey="label" tick={{ fontSize: 11, fill: cLB }} />
                      <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 11, fill: cLB }} width={60} />
                      <Tooltip formatter={(v, name) => [cFmt(Math.round(v)), name]} />
                      <Legend />
                      <Bar dataKey="cashFlow" name="Cash Flow" fill={cAC} radius={[4, 4, 0, 0]} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
              ...
              <div style={cSC}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 4, height: 24, backgroundColor: cVL, borderRadius: 2 }} />
                    <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: cVL }}>Capital Structure</h3>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 12, color: cLB, fontWeight: 600 }}>Year {compCapStructYear}</span>
                      <select value={compCapStructYear} onChange={(e) => setCompCapStructYear(Number(e.target.value))} style={{ fontSize: 12, padding: '2px 6px', border: `1px solid ${cB}`, borderRadius: 4, color: cVL }}>
                        {[0, ...cYearlyData.map(d => d.year)].map(yr => <option key={yr} value={yr}>Year {yr}</option>)}
                      </select>
                    </div>
                    <div style={{ display: 'flex', gap: 16 }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: 10, color: cLB, fontWeight: 600 }}>LTC</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: '#10b981' }}>{Math.round(cCapStructData.ltc)}%</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: 10, color: cLB, fontWeight: 600 }}>DSCR</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: '#10b981' }}>{cCapStructData.dscr.toFixed(2)}x</div>
                      </div>
                    </div>
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: '20px 24px', color: '#fff' }}>
                    ...
                    <div style={{ fontSize: 24, fontWeight: 800 }}>{cFmt(cCapStructData.equity)}</div>
                    <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>{cEquityPct}%</div>
                  </div>
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: '20px 24px', color: '#fff' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    ...
                    <div style={{ fontSize: 24, fontWeight: 800 }}>{cFmt(cCapStructData.debt)}</div>
                    <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>{cDebtPct}%</div>
                  </div>
                </div>
                <div style={{ background: `${cAC}08`, borderRadius: 12, padding: '16px 20px', border: `1px solid ${cAC}20` }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 11, color: cLB, fontWeight: 600 }}>Acquisition Cost</span>
                    <span title="Purchase price + closing costs + capex" style={{ width: 14, height: 14, borderRadius: '50%', border: `1px solid ${cB}`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: cLB, cursor: 'help' }}>?</span>
                  </div>
                  <div style={{ fontSize: 24, fontWeight: 800, color: cAC }}>{cFmt(cAcquisitionCost)}</div>
                </div>
              </div>
              ...
              <div style={cSC}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 4, height: 24, backgroundColor: cVL, borderRadius: 2 }} />
                    <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: cVL }}>Total Investment Return</h3>
                  </div>
                  <div style={{ display: 'flex', border: `1px solid ${cB}`, borderRadius: 6, overflow: 'hidden' }}>
                    <button style={{ padding: '4px 12px', fontSize: 12, fontWeight: 600, background: '#fff', color: cVL, border: 'none', borderRight: `1px solid ${cB}`, cursor: 'pointer' }}>$</button>
                    <button style={{ padding: '4px 12px', fontSize: 12, fontWeight: 600, background: '#f9fafb', color: cLB, border: 'none', cursor: 'pointer' }}>$/sq. ft</button>
                  </div>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: cVL, marginBottom: 12 }}>Profit Breakdown</div>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <tbody>
                    {[
                      { label: 'Annual Cash Flows', value: cCumCashFlows, tip: 'Sum of all annual cash flows over hold period', color: cVL },
                      { label: 'Net Sale Price', value: cNetSalePrice, tip: 'Gross sale price at exit', color: cVL },
                      { label: 'Loan Balance at Exit', value: -Math.abs(cLoanBalAtExit), tip: 'Remaining loan balance paid off at sale', color: '#ef4444' },
                    ].map((r, i) => (
                      <tr key={i}>
                        <td style={{ padding: '8px 0', color: cLB, fontWeight: 500 }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            {r.label}
                            <span title={r.tip} style={{ width: 14, height: 14, borderRadius: '50%', border: `1px solid ${cB}`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: cLB, cursor: 'help' }}>?</span>
                          </span>
                        </td>
                    ...
                    <tr style={{ borderTop: `2px solid ${cB}` }}>
                      <td style={{ padding: '10px 0', fontWeight: 700, color: cVL }}>Total Cash Received</td>
                      <td style={{ padding: '10px 0', textAlign: 'right', fontWeight: 800, color: cVL }}>{cFmt(Math.round(cCumCashFlows + cNetSalePrice - Math.abs(cLoanBalAtExit)))}</td>
                    </tr>
                    <tr><td colSpan={2} style={{ height: 8 }}></td></tr>
                    {[
                      { label: 'Purchase Price', value: purchasePrice, color: cVL },
                      { label: 'Closing Cost and Fees', value: cClosingCosts, tip: 'Estimated closing costs (legal, title, etc.)', color: cVL },
                      { label: 'Initial Investment', value: cInitialInvestment, tip: 'Additional upfront capital investment', color: cVL },
                      { label: 'Financed By Debt', value: -cFinancedByDebt, tip: 'Loan proceeds used to fund acquisition', color: '#10b981' },
                    ].map((r, i) => (
                      <tr key={i}>
                        <td style={{ padding: '8px 0', color: cLB, fontWeight: 500 }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            {r.label}
                            {r.tip && <span title={r.tip} style={{ width: 14, height: 14, borderRadius: '50%', border: `1px solid ${cB}`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: cLB, cursor: 'help' }}>?</span>}
                          </span>
                        </td>
                        <td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 600, color: r.color }}>{r.value < 0 ? `$-${Math.abs(r.value).toLocaleString()}` : cFmt(Math.round(r.value))}</td>
                      </tr>
                    ...
                    <tr style={{ borderTop: `2px solid ${cB}` }}>
                      <td style={{ padding: '10px 0', fontWeight: 700, color: cVL }}>Total Cash Invested</td>
                      <td style={{ padding: '10px 0', textAlign: 'right', fontWeight: 800, color: cVL }}>{cFmt(Math.round(cTotalCashInvested > 0 ? cTotalCashInvested : cTotalEquity))}</td>
                    </tr>
                    <tr style={{ backgroundColor: `${cAC}08`, borderRadius: 8 }}>
                      <td style={{ padding: '12px 8px', fontWeight: 800, color: cAC, borderRadius: '8px 0 0 8px' }}>Total Profit</td>
                      <td style={{ padding: '12px 8px', textAlign: 'right', fontWeight: 800, fontSize: 16, color: cCompTotalProfit >= 0 ? '#10b981' : '#ef4444', borderRadius: '0 8px 8px 0' }}>{cFmt(Math.round(cCompTotalProfit > 0 ? cCompTotalProfit : cTotalProfit))}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
