...
// Highlight cells with anomalous values (>2x or <0.5x section median)
...
export default function T12AuditorTab({ scenarioData, dealId }) {
  ...
  const [activeSubTab, setActiveSubTab] = useState('t12'); // 't12' | 'insights'
  const [highlightedCells, setHighlightedCells] = useState({}); // { row-col: true }
  const tableRef = useRef(null);

  // Check if we already have cached T-12 data in scenarioData
  useEffect(() => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      console.error('T-12 extraction failed:', err);
    ...
    t12Data.sections.forEach((section) => {
      ...
      (section.lines || []).forEach((line) => {
        ...
        rows.push({
          ...
          values: line.values || new Array(12).fill(0),
          annual,
          rowIdx: rowIdx++,
        });
      });
      ...
      if (section.subtotal_label) {
        ...
        rows.push({
          type: 'subtotal',
          label: section.subtotal_label,
          values: section.subtotal_values || new Array(12).fill(0),
          annual,
          rowIdx: rowIdx++,
        });
      }
  ...
  const SeverityBadge = ({ severity }) => {
    ...
    const c = colors[Math.min(severity, 4)] || colors[3];
    return (
      <span style={{
  ...
  // When user clicks a finding, highlight affected months in the table
  const handleFindingClick = (finding) => {
    ...
    finding.affected_months.forEach((month) => {
      // Highlight entire column for that month
    ...
    <div style={{ padding: '24px', backgroundColor: '#f9fafb', minHeight: '100vh', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: '1600px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 11, color: LB, marginBottom: 2 }}>T-12: {sourceFile}</div>
          </div>
          <button
          ...
          >
            ...
            ) : t12Data ? '⟳ Re-analyze' : '⚡ Generate T-12 Analysis'}
          </button>
        </div>
        ...
        {error && (
          <div style={{ ...card, borderLeft: '4px solid #dc2626', backgroundColor: '#fef2f2' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#dc2626', marginBottom: 4 }}>Analysis Failed</div>
        ...
        {!t12Data && !loading && !error && (
          <div style={{ ...card, textAlign: 'center', padding: '60px 28px' }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: VL, marginBottom: 8 }}>T-12 Operating Statement Analysis</div>
            <div style={{ fontSize: 13, color: LB, maxWidth: 500, margin: '0 auto', lineHeight: 1.7 }}>
              Click <strong>"Generate T-12 Analysis"</strong> to extract monthly income and expense data from your uploaded document,
              displayed in an interactive spreadsheet with AI-powered anomaly detection.
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 28 }}>
              {[
                { icon: '📋', label: 'Monthly Spreadsheet', desc: 'Mo 1–12 line items' },
                { icon: '🔍', label: 'AI Audit', desc: 'Anomaly detection' },
                { icon: '⚠️', label: 'Red Flags', desc: 'Ranked by severity' },
              ].map((f, i) => (
                <div key={i} style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 28, marginBottom: 6 }}>{f.icon}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: VL }}>{f.label}</div>
                  <div style={{ fontSize: 11, color: LB }}>{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {loading && (
          <div style={{ ...card, textAlign: 'center', padding: '60px 28px' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: VL, marginBottom: 4 }}>Analyzing T-12 Statement...</div>
            <div style={{ fontSize: 12, color: LB }}>Extracting monthly data & running anomaly detection</div>
          </div>
        )}
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 2, marginBottom: 16, backgroundColor: '#f1f5f9', borderRadius: 10, padding: 3, width: 'fit-content' }}>
              {[
                ...
                { id: 'insights', label: 'Insights', badge: auditFindings.length || null },
              ].map((tab) => (
                <button
                  key={tab.id}
              ...
              <div ref={tableRef} style={{ ...card, padding: 0, overflow: 'hidden' }}>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, tableLayout: 'fixed', minWidth: 1400 }}>
                    <colgroup>
                      <col style={{ width: 40 }} />  {/* row # */}
                      <col style={{ width: 280 }} /> {/* description */}
                      {months.map((_, i) => <col key={i} style={{ width: 90 }} />)}
                      <col style={{ width: 100 }} /> {/* annual */}
                    </colgroup>
                    <thead>
                      <tr style={{ backgroundColor: '#f8fafc', borderBottom: `2px solid ${B}` }}>
                        ...
                        {months.map((m, i) => (
                          <th key={i} style={{
                            padding: '10px 6px', textAlign: 'right', fontWeight: 600, color: '#374151',
                            borderRight: `1px solid ${B}`, fontSize: 10,
                          }}>
                            {m}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {spreadsheetRows.map((row) => {
                        ...
                        const borderB = isBold ? `2px solid ${isNOI ? '#818cf8' : B}` : `1px solid #f1f5f9`;

                        return (
                          <tr key={row.rowIdx} style={{ backgroundColor: bgColor }}>
                            {/* Row number */}
                            <td style={{ padding: '6px', textAlign: 'center', fontSize: 10, color: '#9ca3af', borderRight: `1px solid ${B}`, borderBottom: borderB }}>
                              {row.rowIdx + 1}
                            </td>
                            {/* Description */}
                            <td style={{
                              padding: '6px 8px', fontWeight: isBold ? 700 : 400,
                              color: isNOI ? AC : VL, borderRight: `1px solid ${B}`, borderBottom: borderB,
                              fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                            }}>
                              {row.type === 'line' ? (
                                <span>
                                  {row.code && <span style={{ color: LB, marginRight: 6, fontSize: 10 }}>{row.code}</span>}
                                  {row.description}
                                </span>
                              ) : (
                                row.label
                            ...
                            {(row.values || []).map((val, mi) => {
                              ...
                              return (
                                <td key={mi} style={{
                                  ...
                                  borderRight: `1px solid ${B}`, borderBottom: borderB,
                            ...
                            <td style={{
                              ...
                              borderBottom: borderB, backgroundColor: isNOI ? '#e0e7ff' : '#f0f9ff',
                              fontSize: 11, fontFamily: 'monospace',
                            }}>
                              {fmtNum(row.annual)}
                            </td>
            ...
            {activeSubTab === 'insights' && (
              <div>
                ...
                {summary && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
                    {[
                      ...
                      { label: 'Critical Issues', value: summary.critical_count || auditFindings.filter(f => f.severity <= 2).length, color: '#dc2626' },
                      { label: 'Data Source', value: summary.data_source === 'extracted_from_document' ? 'Extracted' : 'Generated', color: '#10b981' },
                      { label: 'Data Quality', value: (summary.overall_data_quality || 'N/A').charAt(0).toUpperCase() + (summary.overall_data_quality || 'n/a').slice(1), color: '#f59e0b' },
                    ].map((m, i) => (
                      <div key={i} style={{ ...card, marginBottom: 0, borderLeft: `4px solid ${m.color}`, padding: '14px 18px' }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 16, fontWeight: 700, color: VL }}>
                    Underwriting T-12 abnormalities <span style={{ fontSize: 12, fontWeight: 500, color: LB }}>(ranked by severity)</span>
                  </div>
                </div>
                ...
                {auditFindings.length > 0 ? (
                    ...
                    {auditFindings.map((finding, idx) => (
                      <div
                        ...
                        style={{
                          ...
                          borderLeft: `4px solid ${finding.severity <= 1 ? '#dc2626' : finding.severity <= 2 ? '#ea580c' : finding.severity <= 3 ? '#ca8a04' : '#16a34a'}`,
                          transition: 'all 0.15s',
                        }}
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: VL, marginBottom: 6 }}>
                              {finding.title}
                            </div>
                  ...
                  <div style={{ ...card, textAlign: 'center', padding: '40px 28px' }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, color: VL }}>No anomalies detected</div>
                    <div style={{ fontSize: 12, color: LB, marginTop: 4 }}>The T-12 operating statement appears clean</div>
                  </div>
                )}
              </div>
            )}
